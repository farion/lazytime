...
impl SettingsView {
    ...
    pub fn ui(
        ...
        ui.horizontal(|ui| {
            ...
            {
                match self.edit.to_config(config, self.theme_preference.clone(), self.sidebar_collapsed) {
                    Ok(next) => {
                            ...
                            Ok(json) => {
                                if fs::write(&p, json).is_ok() {
                                    *config = next;
                                    ...
                                    message = Some(format!("error: failed to write {}", p.display()));
                                }
                            }
        ...
        egui::ScrollArea::vertical().show(ui, |ui| {
            ...
            style::field_block(ui, |ui| {
                ...
                ui.add_space(4.0);
                style::setting_row(ui, "Theme", "Choose how the GUI theme is selected.", LABEL_WIDTH, |ui| {
                    egui::ComboBox::from_id_salt("settings_theme")
                        ...
                        .show_ui(ui, |ui| {
                            ui.selectable_value(&mut self.theme_preference, ThemePreference::Auto, "Auto");
            ...
            style::field_block(ui, |ui| {
                ...
                ui.add_space(4.0);
                style::setting_text_row(ui, "Default project", "Project preselected when starting tracking.", LABEL_WIDTH, &mut self.edit.default_project);
                style::setting_text_row(ui, "Tracking stability (seconds)", "How long an app/title match must stay stable before tracking changes.", LABEL_WIDTH, &mut self.edit.tracking_stability_seconds);
                style::setting_text_row(ui, "Reminder interval (seconds)", "How often reminders are shown while tracking is running.", LABEL_WIDTH, &mut self.edit.track_reminder_seconds);
                style::setting_text_row(ui, "Reminder snooze (seconds)", "How long reminders stay snoozed after manual stop.", LABEL_WIDTH, &mut self.edit.track_reminder_snooze_seconds);
                style::setting_text_row(ui, "Summary refresh (seconds)", "How often summary data is refreshed.", LABEL_WIDTH, &mut self.edit.summary_update_seconds);
                style::setting_text_row(ui, "Report start", "Optional report start date/time filter.", LABEL_WIDTH, &mut self.edit.report_start);
                style::setting_text_row(ui, "Report end", "Optional report end date/time filter.", LABEL_WIDTH, &mut self.edit.report_end);
                style::setting_text_row(ui, "Database file", "Path to the LazyTime SQLite database file.", LABEL_WIDTH, &mut self.edit.db_file);
                style::setting_text_row(ui, "IPC socket path", "Socket endpoint used for daemon communication.", LABEL_WIDTH, &mut self.edit.ipc_socket_path);
            });

            style::field_block(ui, |ui| {
                ui.label(egui::RichText::new("Jira").strong().size(20.0));
                ui.add_space(4.0);
                style::setting_text_row(ui, "Jira URL", "Base URL of your Jira instance.", LABEL_WIDTH, &mut self.edit.jira_url);
                style::setting_row(
                    ui,
                    "Jira API token",
                ...
                style::setting_text_row(ui, "Jira email", "Account email used for Jira API access.", LABEL_WIDTH, &mut self.edit.jira_email);
                style::setting_text_row(ui, "Jira project key", "Default Jira project key for synchronization.", LABEL_WIDTH, &mut self.edit.jira_project);
                style::setting_text_row(ui, "Jira assignee", "Optional assignee filter (for example: me).", LABEL_WIDTH, &mut self.edit.jira_assignee);
                style::setting_text_row(ui, "Jira issue type", "Issue type created during sync (for example: Task).", LABEL_WIDTH, &mut self.edit.jira_issue_type);
                style::setting_text_row(ui, "SAP field name", "Jira field key used to store SAP references.", LABEL_WIDTH, &mut self.edit.jira_sap_field);
            });

            style::field_block(ui, |ui| {
                ui.label(egui::RichText::new("Working Hours").strong().size(20.0));
                ui.add_space(4.0);
                style::setting_readonly_row(ui, "Weekly schedule", "Overview of configured ranges per day.", LABEL_WIDTH, &format_working_hours_summary(&self.edit.working_hours));
                if ui
                    .button(style::icon_label(ui, icons::CLOCK_COUNTDOWN, "Edit Working Hours"))
                    .clicked()
                {
        ...
        if self.working_hours_modal {
            self.render_working_hours_modal(ctx);
        }
    ...
    fn render_working_hours_modal(&mut self, ctx: &egui::Context) {
        ...
        egui::Window::new("Working hours")
            ...
            .show(ctx, |ui| {
                egui::Frame::new()
                    ...
                    .show(ui, |ui| {
                        ui.label("Left/Right day, Up/Down range, a:add, d:delete");
                        for (idx, day) in WEEKDAY_NAMES.iter().enumerate() {
                            let marker = if idx == self.day_idx { ">" } else { " " };
                            let ranges = self
                                .edit
                                .working_hours
                                .get(&(idx as u8))
                            ...
                            for (ri, r) in ranges.iter().enumerate() {
                                if idx == self.day_idx && ri == self.range_idx {
                                    txt.push(format!("[{}-{}]", r.start, r.end));
                                } else {
                                    txt.push(format!("{}-{}", r.start, r.end));
                                }
                            }
                            ui.label(format!("{} {:<3} | {}", marker, day, txt.join("  ")));
                        }
                        ...
                        if ctx.input(|i| i.key_pressed(egui::Key::D))
                            && let Some(ranges) = self.edit.working_hours.get_mut(&(self.day_idx as u8))
                            && !ranges.is_empty()
                        {
                            let idx = self.range_idx.min(ranges.len() - 1);
                            ranges.remove(idx);
                            self.range_idx = self.range_idx.saturating_sub(1);
                        }

                        ui.separator();
                        if let Some(ranges) = self.edit.working_hours.get_mut(&(self.day_idx as u8))
                            && let Some(r) = ranges.get_mut(self.range_idx)
                        {
                            ui.horizontal(|ui| {
                                ui.label("Start");
                                style::padded_text_edit(ui, &mut r.start);
                            });
                            ui.horizontal(|ui| {
                                ui.label("End");
                                style::padded_text_edit(ui, &mut r.end);
                            });
                        }
