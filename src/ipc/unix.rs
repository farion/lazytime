...
pub async fn run_ipc_server(endpoint: &str, tx_reload: mpsc::Sender<String>) -> Result<()> {
    ...
    if socket_path.exists() {
        std::fs::remove_file(socket_path)
            .with_context(|| format!("failed to remove stale socket {}", endpoint))?;
    }
    if let Some(parent) = socket_path.parent() {
        std::fs::create_dir_all(parent)?;
    }
    let listener = UnixListener::bind(socket_path)
        .with_context(|| format!("failed to bind ipc socket {}", endpoint))?;
...
async fn handle_client(stream: UnixStream, tx_reload: mpsc::Sender<String>) -> Result<()> {
...
pub async fn notify_projects_updated(endpoint: &str, timestamp: &str) -> Result<()> {
    let mut stream = UnixStream::connect(endpoint)
        .await
        .with_context(|| format!("failed to connect IPC socket {}", endpoint))?;
